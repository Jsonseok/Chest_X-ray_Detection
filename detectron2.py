...
Original file is located at
    https://colab.research.google.com/drive/1SP_6UywX1Z4lziHM-pl4wbdJ7RwBZ6_T
"""
...
with zipfile.ZipFile('/content/drive/MyDrive/1024x1024.zip', 'r') as zip_ref:
  zip_ref.extractall('./vinbigdata-chest-xray-resized-png-1024x1024')
...
# --- Read data ---
inputdir = Path("/content")
datadir = inputdir / "vinbigdata-chest-xray-resized-png-1024x1024"
...
inputdir = Path("/content")
traineddir = inputdir / "drive/MyDrive"

# flags = Flags()
flags: Flags = Flags().update(load_yaml("/content/results/v9/flags.yaml"))
print("flags", flags)
debug = flags.debug
# flags_dict = dataclasses.asdict(flags)
outdir = Path(flags.outdir)
...
imgdir = inputdir / "vinbigdata-chest-xray-resized-png-1024x1024"

# Read in the data CSV files

test_meta = pd.read_csv('/content/drive/MyDrive/test_meta.csv')
sample_submission = pd.read_csv('/content/drive/MyDrive/sample_submission (6).csv')
...
cfg.MODEL.WEIGHTS = "/content/results/v9/model_final.pth"
...
DatasetCatalog.register(
    "test", lambda: get_vinbigdata_dicts_test(imgdir, test_meta, debug=debug)
)
...
submission_det.to_csv(outdir/"detectron2_submission_all_train_include_14_63000.csv", index=False)
submission_det

from google.colab import files
files.download('/content/results/v9/detectron2_submission_all_train_include_14_63000.csv')

from google.colab import files
files.download('/content/results/v9/model_final.pth')

from google.colab import files
files.download('/content/results/v9/flags.yaml')
...
# flags = Flags()
flags: Flags = Flags().update(load_yaml("/content/drive/MyDrive/flags_detectron2_submission_all_train_no_14_55000.yaml"))
print("flags", flags)
debug = flags.debug
# flags_dict = dataclasses.asdict(flags)
outdir = Path(flags.outdir)
...
# --- Read data ---
datadir = inputdir / "vinbigdata-chest-xray-abnormalities-detection"
if flags.imgdir_name == "/vinbigdata-chest-xray-resized-png-1024x1024":
...
# cfg now already contains everything we've set previously. We changed it a little bit for inference:
# path to the model we just trained
cfg.MODEL.WEIGHTS = "/content/drive/MyDrive/model_final_detectron2_submission_all_train_no_14_55000.pth"
print("Original thresh", cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST)  # 0.05
cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = 0.0  # set a custom testing threshold
print("Changed  thresh", cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST)
predictor = DefaultPredictor(cfg)
...
# This submission includes only detection model's predictions
submission_det = pd.DataFrame(results_list, columns=['image_id', 'PredictionString'])
submission_det.to_csv(outdir/"detectron2_submission_all_train_no_14_55000(check).csv", index=False)
submission_det